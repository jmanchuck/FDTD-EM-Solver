...
class Solver:
    ...
    def save_to_json(self):
        ...
        for pulse in self.pulses:
            self.save_dict_json['pulses'].append({
                'sigma_w': 0 if pulse.sigma_w is None else pulse.sigma_w,
    ...
    def add_gaussian_pulse(self, sigma_w, location, start_time=0, direction=None):
        if 3 * sigma_w > self.omega_max:
            ...
            self.update_constants()
        index_location = [self.convert(i) for i in location]
        new_pulse = Pulse(sigma_w=sigma_w, dt=self.dt, location=index_location, start_time=start_time, type="gd",
    ...
    def add_sinusoidal_pulse(self, omega_0, location, start_time=0, direction=None):
        if omega_0 > self.omega_max:
            self.omega_max = omega_0
            self.update_constants()

        index_location = [self.convert(i) for i in location]
        new_pulse = Pulse(dt=self.dt, location=index_location, omega_0=omega_0, start_time=start_time,
                          type="sinusoidal", sim_end_time=self.end_time, direction=direction, real_location=location)
        self.pulses.append(new_pulse)
    ...
    def update(self, time):
        ...
        for pulse in self.pulses:
            if pulse.get_start_time() < time < pulse.get_end_time() and not pulse.get_direction() is None:
                if pulse.get_direction() == "right":
                    self.h[:, pulse.get_col()] = h_prev[:, pulse.get_col()] + self.mu_coefficient_arr[:,
                                                                              pulse.get_col()] * \
                                                 ((ex_prev[1:, pulse.get_col()] - ex_prev[:-1, pulse.get_col()]) -
                                                  (ey_prev[:, 1 + pulse.get_col()] -
                                                   (ey_prev[:, pulse.get_col()] + pulse.magnitude(
                                                       self.step) * math.sqrt(
                                                       MU / EPSILON))))
        ...
        for pulse in self.pulses:
            if pulse.get_start_time() < time < pulse.get_end_time() and not pulse.get_direction() is None:
                if pulse.get_direction() == "right":
                    self.ey[:, pulse.get_col()] = ey_prev[:, pulse.get_col()] - self.eps_coefficient_arr[:,
                                                                                pulse.get_col()] * (
                                                          self.h[:, pulse.get_col()] - pulse.magnitude(self.step) - (
                                                  self.h[:, pulse.get_col() - 1]))
        ...
        if len(self.recorded_energies) > 0:
            for energy_record_obj in self.recorded_energies:
                storage_list = energy_record_obj["energies"]
                i = energy_record_obj["i"]
                j = energy_record_obj["j"]
                storage_list.append(0.5 * ((self.material.eps_arr[i][j] *
                                            (self.ex[i][j] ** 2 +
                                             self.ey[i][j] ** 2) +
                                            self.material.mu_arr[i][j] *
                                            self.h[i][j] ** 2)))
    ...
    def solve(self, realtime=True, step_frequency=5, solve_only=False):
    ...
    def plot_and_show(self):
        ...
        im = ax1.imshow(self.h, animated=True, extent=[0, self.length_x, self.length_y, 0], vmax=self.pulses_max,
                        vmin=-self.pulses_max, aspect='auto', cmap='seismic')
    ...
    def record_energy(self, location):
        row, col = [self.convert(x) for x in location]
        self.recorded_energies.append({"i": row, "j": col, "location": location, "energies": []})
...
class MaterialArray:
    ...
    def set_quarter_circle(self, center, radius, epsilon_rel, mu_rel=1, thickness=0, convert=True):
        ...
        print("latest one   ")
        x, y = np.mgrid[-center_i:self.size - center_i, -center_j:self.size - center_j]
        circ_mask = np.logical_and(radius ** 2 > x ** 2 + y ** 2, x ** 2 + y ** 2 > (radius - thickness) ** 2)
        quarter = np.logical_and(x < 0, y > 0)
        circ_mask = circ_mask & quarter

        self.eps_arr[circ_mask] = epsilon_rel * EPSILON
        self.mu_arr[circ_mask] = mu_rel * MU
        ...
        for i in range(radius):
            for j in range(radius):
                if thickness == 0:
                    ...
                    if radius ** 2 + 100 >= i ** 2 + j ** 2 >= (radius - thickness) ** 2:
                        self.eps_arr[center_i - i][center_j + j] = epsilon_rel * EPSILON
                        self.mu_arr[center_i - i][center_j + j] = mu_rel * MU
    ...
    def set_waveguide(self, start_point, thickness, rect_length, radius, epsilon_rel, mu_rel=1, convert=True):
        ...
        self.set_material_rect((start_i, start_j), (start_i + thickness, start_j + rect_length), epsilon_rel,
                               mu_rel=mu_rel, convert=False)

        quarter_circle_center = (start_i + radius, start_j + rect_length)
        self.set_quarter_circle(quarter_circle_center, radius, epsilon_rel, thickness=thickness, mu_rel=mu_rel,
                                convert=False)

        down_rect_start = (quarter_circle_center[0], quarter_circle_center[1] + radius - thickness)
        self.set_material_rect(down_rect_start, (down_rect_start[0] + rect_length, down_rect_start[1] + thickness),
                               epsilon_rel, mu_rel=mu_rel, convert=False)

    def plot(self):
        fig, ax = plt.subplots()
        image = ax.imshow(self.eps_arr, extent=([0, self.length_x, self.length_y, 0]), vmax=(np.max(self.eps_arr)),
                          vmin=0, aspect='auto')
...
class Pulse:
    ...
    def __init__(self, dt, start_time, type, sim_end_time, sigma_w=None, location=None, real_location=None, omega_0=0,
                 direction=None, step_frequency=1):
        print("--- Pulse Info ---")
        TYPES = ["gd", "oscillate", "sinusoidal"]
        ...
        self._type = type
        self.plot_title = "Oscillating pulse" if self._type == "oscillate" else "Gaussian derivative"
        self.step_frequency = step_frequency
        ...
        if self.sigma_w is not None:
            ...
            self.pulse_mid = 3 * self.sigma_t  # the middle of the pulse (approximately 3 standard deviations)
            self._end_time = self._start_time + self.pulse_mid * 3
            self._omega_max = 3 * self.sigma_w + self.omega_0
            print(
                "Pulse type: {}, sigma_w: {}, omega_0: {}, omega_max: {}".format(self._type, self.sigma_w, self.omega_0,
                                                                                 self._omega_max))

        else:
            self._end_time = self._sim_end_time
            self._omega_max = self.omega_0
            print("Pulse type: {}, omega_0: {}".format(self._type, self.omega_0))
    ...
    def create_magnitude_vector(self):
        t = self._time_vector
        if self._type == "gd":
            self._magnitude_vector = (-t + self.pulse_mid) * (1 / (self.sigma_t * np.sqrt(2 * math.pi))) * (
                np.exp(-((t - self.pulse_mid) ** 2) / (2 * (self.sigma_t ** 2))))
        elif self._type == "oscillate":
            self._magnitude_vector = np.cos(t * self.omega_0) * (1 / (self.sigma_t * np.sqrt(2 * math.pi))) * (
                np.exp(-((t - self.pulse_mid) ** 2) / (2 * (self.sigma_t ** 2))))
        elif self._type == "sinusoidal":
            self._magnitude_vector = np.sin(t * self.omega_0)
            return

        print("Pulse duration: {} seconds, {} steps".format(self._time_vector[-1], len(self._time_vector)))

    def plot_time(self, scaled_plot=True, show=True, title=False):
        ...
        if self._type == "oscillate":
            ...
            envelope = (1 / (self.sigma_t * np.sqrt(2 * math.pi))) * (
                np.exp(-((local_time_vector - self.pulse_mid) ** 2) / (2 * (self.sigma_t ** 2))))
    ...
    def plot_frequency(self, show=True, title=False):
        ...
        if self._type == "oscillate":
            plot_vector = self.sigma_t * 0.5 * (
                        np.exp((-(frequencies_vector - self.omega_0) ** 2 / (2 * self.sigma_w ** 2))) +
                        np.exp((-(frequencies_vector + self.omega_0) ** 2 / (2 * self.sigma_w ** 2))))
            plt.plot(frequencies_vector, plot_vector)
        elif self._type == "gd":
            plot_vector = np.abs(frequencies_vector) * self.sigma_t * np.exp(
                -((frequencies_vector ** 2) / (2 * self.sigma_w ** 2)))
            plt.plot(frequencies_vector, plot_vector)
    ...
    def plot_frequency_fft(self, show=True, title=False):
...
def test():
    ...
    solver = Solver(points_per_wavelength=s, stability=stability, eps_r_max=4, mu_r_max=3, simulation_size=3,
    ...
    mat.set_material_rect((2, 2), (2.8, 2.8), 3, 3)

    solver.solve(step_frequency=50)
